...
export const useTabStore = create<TabStore>((set, get) => ({
  ...
  setTabs: (tabs) => ErrorBoundary.wrapSync(
    () => {
      // Validate all tabs
  ...
  addTab: (tab) => ErrorBoundary.wrapSync(
    () => {
      if (!DataValidator.validateTab(tab)) {
  ...
  removeTab: (tabId) => ErrorBoundary.wrapSync(
    () => {
  ...
  updateTab: (tabId, updates) => ErrorBoundary.wrapSync(
    () => {
      if (typeof tabId !== 'number') {
        console.error('Invalid tabId:', tabId)
        return
  ...
  setGroups: (groups) => ErrorBoundary.wrapSync(
    () => {
      // Validate groups
  ...
  addGroup: (group) => ErrorBoundary.wrapSync(
    () => {
      if (!group.id || !group.name || !Array.isArray(group.tabIds)) {
  ...
  removeGroup: (groupId) => ErrorBoundary.wrapSync(
    () => {
  ...
  updateGroup: (groupId, updates) => ErrorBoundary.wrapSync(
    () => {
      if (typeof groupId !== 'string') {
        console.error('Invalid groupId:', groupId)
        return
  ...
  setActiveTab: (activeTabId) => ErrorBoundary.wrapSync(
    () => {
