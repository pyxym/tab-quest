...
export class TabTracker {
  ...
  private static async updateDailyStats(category: string, domain: string, timeSpent: number) {
    ...
    const productiveTime = (todayStats.categoryBreakdown['work'] || 0) + 
                          (todayStats.categoryBreakdown['productivity'] || 0)
    const distractingTime = (todayStats.categoryBreakdown['social'] || 0) + 
                           (todayStats.categoryBreakdown['entertainment'] || 0)
