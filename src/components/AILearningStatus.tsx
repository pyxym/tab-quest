      ...
      <div className="inline-flex items-center gap-2 px-2 py-1 glass-card">
        ...
        <div className="relative w-6 h-6">
          <svg className="w-6 h-6 -rotate-90">
            {/* 배경 원 */}
            <circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2" fill="none" className="text-white/20" />
            {/* 진행률 원 */}
            <circle
              cx="12"
              cy="12"
        ...
        <div className="flex flex-col">
          ...
          <span className="text-[10px] glass-text opacity-60">{stats.totalDomains} domains</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="glass-card">
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        {/* 새로고침 버튼 */}
        <button onClick={loadAIStats} disabled={isLoading} className="text-xs glass-text opacity-60 hover:opacity-100">
      ...
      <div className="flex items-center justify-center mb-4">
        <div className="relative w-20 h-20">
          <svg className="w-20 h-20 -rotate-90">
            {/* 배경 원 */}
            <circle cx="40" cy="40" r="36" stroke="currentColor" strokeWidth="4" fill="none" className="text-white/20" />
            {/* 진행률 원 */}
            <circle
              cx="40"
              cy="40"
      ...
      {/* 마지막 업데이트 시간 */}
      <p className="text-[10px] glass-text opacity-40 text-center mt-2">Last updated: {stats.lastUpdated}</p>
    </div>
  );
};
